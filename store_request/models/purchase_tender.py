...
class PurchaseTender(models.Model):
    ...
    purchase_bid= fields.Boolean(default=True, compute="_purchase_type_check",store=False)
    ...
    def _purchase_type_check(self):
        reqeust_id = self.env['purchase.request'].search([('name','=',self.request_id)])
        self.purchase_bid =  reqeust_id.purchase_type == "bid"
    ...
    def _compute_purchase_order_count(self):
        for rec in self:
            rec.purchase_order_count = self.env['purchase.order'].search_count([('request_id', '=', rec.request_id)])
    ...
    def create_purchase_order(self):
        for tender in self:
            if tender.purchase_bid:
                if not tender.memo or not tender.comeetes:
                    raise ValidationError("Please Attach Comeetes memo!")
